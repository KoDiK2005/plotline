...
import { getStoryStats, type StoryStats } from './traverse'
...
interface AchievementContext {
  ...
  statsByStoryId: Map<string, StoryStats>
}

// getStoryStats() walks the whole story graph (BFS), so cache it per story
// instead of recomputing it for every achievement that needs it below.
function statsFor(ctx: AchievementContext, story: Story): StoryStats {
  return ctx.statsByStoryId.get(story.id)!
}
...
const ACHIEVEMENTS: Achievement[] = [
  ...
  },
  {
    ...
    isUnlocked: (ctx) =>
      ctx.stories.some((s) => {
        const total = statsFor(ctx, s).endingCount
  ...
  },
  {
    ...
    getProgress: (ctx) => {
      const withEndings = ctx.stories.filter((s) => statsFor(ctx, s).endingCount > 0)
  ...
  },
  {
    ...
    isUnlocked: (ctx) => ctx.stories.some((s) => statsFor(ctx, s).nodeCount >= 10),
    getProgress: (ctx) => {
      if (ctx.stories.length === 0) return null
      const max = ctx.stories.reduce((m, s) => Math.max(m, statsFor(ctx, s).nodeCount), 0)
...
export function computeAchievements(
  ...
  const statsByStoryId = new Map(stories.map((s) => [s.id, getStoryStats(s)]))
  const ctx: AchievementContext = { stories, progress, statsByStoryId }
